    ...
    return <div className="hb-network-statistics">
        <Row className="row pb-lg-1 mb-3 mb-lg-4">
            <Col xl={6} lg={8} className="mb-lg-0 mb-3">
                <HBStatisticCard label="Your investment package" multiple value={`${get(myPackage, 'dot_amount', 0)} DOT`} subValue={`Level ${PACKAGE_RANKING_TYPES[get(myPackage, 'profit_type', 5)]}`} value2={`Lock ${get(myPackage, 'him_amount', 0)} HIM`} subValue2={`Plan ${get(myPackage, 'plan', 0)}`} className="highlight" />
            </Col>
            <Col xl={3} lg={4} md={6}>
