export interface DicOption {
    [key: number]: string;
}

// export const HIMBOX_POOL_CONTRACT: string = '0xa98C029bE421E25f9CA74A4E1C774a08bd670260';
// export const DOT_ADDRESS: string = '0xFfFFfFff1FcaCBd218EDc0EbA20Fc2308C778080';
// export const NETWORK_SCAN: string = 'https://moonbeam.moonscan.io';
// export const USDT_ADDRESS: string = '0xAa36B80Bed8F260496f7561D49C7a407931D194c';
// export const HIM_PRIVATE_SALE_CONTRACT: string = '0xC199404e8B340B40c6E7Fa4488A155E3c252018f';
// export const chainIdsSupport = [1284]; //mainnet moonbeam
// export const chainIdDefaults = 1284;
// export const SUPPORTED_CHAIN_RPC: any = ['https://rpc.api.moonbeam.network'];


export const HIMBOX_POOL_CONTRACT: string = '0x8D4313413b8Af1467900EcA394A89237E35f8cD8';
export const DOT_ADDRESS: string = '0x08f43498D7870C98501213A9d18EfcdD8f77c42F';
export const NETWORK_SCAN: string = 'https://testnet.snowtrace.io';
export const USDT_ADDRESS: string = '0x3dA82dDEAf8b6a06F261Bc2CBe417dF7D62a5059';
export const HIM_PRIVATE_SALE_CONTRACT: string = '0x9a2952d20bCd499a1484BB4c3D675BF2794D6a1A';
export const chainIdsSupport = [43113]; //testnet avax
export const chainIdDefaults = 43113;
export const SUPPORTED_CHAIN_RPC: any = ['https://api.avax-test.network/ext/bc/C/rpc'];
